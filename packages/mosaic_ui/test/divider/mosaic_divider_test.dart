...
void main() {
  testWidgets('horizontal divider has the configured thickness', (
    ...
    await tester.pumpWidget(
      _wrap(const SizedBox(width: 100, child: MosaicDivider(thickness: 3))),
    );
    final box = tester.widget<SizedBox>(
      find.byWidgetPredicate((w) => w is SizedBox && w.height == 3),
    );
    expect(box.height, 3);
  });

  testWidgets('vertical divider has the configured thickness', (tester) async {
    await tester.pumpWidget(
      _wrap(
        const SizedBox(
          height: 100,
    ...
    final box = tester.widget<SizedBox>(
      find.byWidgetPredicate((w) => w is SizedBox && w.width == 2),
    );
    expect(box.width, 2);
  });
  ...
  testWidgets('color override beats divider token', (tester) async {
    ...
    await tester.pumpWidget(
      _wrap(const SizedBox(width: 100, child: MosaicDivider(color: override))),
    );

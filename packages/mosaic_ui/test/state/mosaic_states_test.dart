...
Widget _wrap(Widget child) => MosaicTheme.test(
  child: Directionality(
    ...
    child: Center(child: SizedBox(width: 360, height: 480, child: child)),
  ),
);
...
void main() {
  ...
  group('MosaicErrorState', () {
    testWidgets('shows retry button when callback is supplied', (tester) async {
      var called = false;
      await tester.pumpWidget(
        _wrap(
          MosaicErrorState(
    ...
    testWidgets('omits retry button without callback', (tester) async {
      await tester.pumpWidget(_wrap(const MosaicErrorState(body: 'fail')));
  ...
  group('MosaicInlineError', () {
    testWidgets('renders message in error tone', (tester) async {
      await tester.pumpWidget(_wrap(const MosaicInlineError('Required field')));
